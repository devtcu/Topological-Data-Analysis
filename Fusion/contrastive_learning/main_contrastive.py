#This is a work in progress, do not run it yet!
...
def visualize_contrastive_results(data, model, save_path=None):
    """visualization of contrastive coordinate prediction results"""
    model.eval()
    with torch.no_grad():
    ...
    # First plot is just original data with hidden nodes shown
    plt.subplot(1, 3, 1)
    ...
    # second plot is redicted coordinates
    plt.subplot(1, 3, 2)
    plt.scatter(true_coords[visible_mask, 0], true_coords[visible_mask, 1], 
               c='blue', alpha=0.6, s=20, label='Visible nodes')
    ...
    # Third plot is Error visualization
    plt.subplot(1, 3, 3)
    plt.scatter(true_coords[visible_mask, 0], true_coords[visible_mask, 1], 
               c='blue', alpha=0.3, s=10, label='Visible nodes')
    
    # A visual of the prediction errors as lines
